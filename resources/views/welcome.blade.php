<x-Layout :alamat="$profile->alamat_kantor">
    <!-- Hero Section -->
    <section class="relative h-screen bg-cover bg-center opacity-30" style="background-image: url('{{ asset('storage/' . $profile->profile_pic) }}')">
    ...
    <div class="absolute inset-0 z-10 flex items-center justify-center px-4 sm:px-6 lg:px-8">
        <div class="text-center space-y-6 max-w-3xl mx-auto">
            <h1 class="text-4xl font-extrabold tracking-tight text-gray-800 dark:text-white md:text-5xl lg:text-6xl">
                Sampaikan <span class="text-blue-600 dark:text-blue-500">aspirasimu.</span> Bantu kami membenahi lingkungan {{ $profile->nama_kelurahan }}.
            </h1>
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div class="bg-gray-50 dark:bg-gray-800 p-8 rounded-lg shadow-md text-center transition-transform hover:scale-105">
                    <h3 class="text-3xl font-bold text-gray-900 dark:text-white">{{ $report['total_count'] ?? 0 }}</h3>
                    <p class="mt-2 text-gray-500 dark:text-gray-400 text-lg">Total Laporan</p>
                </div>
                <div class="bg-gray-50 dark:bg-gray-800 p-8 rounded-lg shadow-md text-center transition-transform hover:scale-105">
                    <h3 class="text-3xl font-bold text-gray-900 dark:text-white">{{ $report['report_non_done'] }}</h3>
                    <p class="mt-2 text-gray-500 dark:text-gray-400 text-lg">Laporan Selesai</p>
                </div>
                <div class="bg-gray-50 dark:bg-gray-800 p-8 rounded-lg shadow-md text-center transition-transform hover:scale-105">
                    <h3 class="text-3xl font-bold text-gray-900 dark:text-white">{{ $profile->total_populasi }}</h3>
                    <p class="mt-2 text-gray-500 dark:text-gray-400 text-lg">Total Populasi</p>
                </div>
            </div>
        </div>
    </section>
